...
# define __NETV 0x79C2 L

#endif

#if defined(__NETV)#include <fstream>
...
class Server {
  ...
  public:
    ...
    explicit Server(uint _port): io_context(),
  port(_port),
  ...
  void run() {
    ...
    if (err_code == 0) {
      ...
      if (_tmp == 'y'
        ...
        if (if_success == 0) {
          spdlog::info("NodeJS has been installed  successfully");
        } else {
      ...
      [this, socket, message, & host_data](const boost::system::error_code & error, std::size_t length) {
        if (!error) {
          if (length > __MAX_CHAR_PER_MESSAGE) {
            ...
            for (const auto & sock: sockets) {
              if (sock != socket) {
                if (message -> substr(0, 5) == "/send"
                  or message -> substr(0, 6) == "/hello"
                  or message -> substr(0, 6) == "/socks"
                  or message -> substr(0, 7) == "/online"
  ...
  std::shared_ptr < tcp::socket >> data;
  /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
